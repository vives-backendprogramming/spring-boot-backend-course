...
- [📋 Complete Logging Example](#-complete-logging-example)
...
public class PizzaService {
    ...
    private final PizzaRepository pizzaRepository;
    private final PizzaStoreConfig config;
    
    public PizzaService(PizzaRepository pizzaRepository, PizzaStoreConfig config) {
        this.pizzaRepository = pizzaRepository;
        this.config = config;
        log.info("PizzaService initialized for environment: {}", config.getEnvironment());
    }
    
    public List<Pizza> getAllPizzas() {
        log.debug("Retrieving all pizzas");
        List<Pizza> pizzas = pizzaRepository.findAll();
        log.info("Retrieved {} pizzas", pizzas.size());
        return pizzas;
    }
    ...
    public Pizza getPizzaById(Long id) {
        ...
        return pizzaRepository.findById(id)
            .map(pizza -> {
                log.info("Found pizza: {} (€{})", pizza.getName(), pizza.getPrice());
                return pizza;
            })
            .orElseThrow(() -> {
                log.error("Pizza not found with id: {}", id);
                return new RuntimeException("Pizza not found with id: " + id);
            });
    }
    ...
    public Pizza createPizza(Pizza pizza) {
        ...
        if (pizza.getPrice() == null || pizza.getPrice().compareTo(BigDecimal.ZERO) <= 0) {
            log.warn("Invalid price for pizza {}, setting default", pizza.getName());
            pizza.setPrice(new BigDecimal("10.00"));
        }
        
        try {
            Pizza saved = pizzaRepository.save(pizza);
            log.info("Pizza created successfully - ID: {}, Name: {}", saved.getId(), saved.getName());
            return saved;
        } catch (Exception e) {
            log.error("Failed to create pizza: {}", pizza.getName(), e);
            throw new RuntimeException("Could not create pizza", e);
        }
    }
    ...
    public void deletePizza(Long id) {
        ...
        if (!pizzaRepository.findById(id).isPresent()) {
            log.warn("Attempted to delete non-existent pizza with id: {}", id);
            throw new RuntimeException("Pizza not found");
        }
        
        pizzaRepository.deleteById(id);
        log.info("Pizza deleted successfully: {}", id);
    }
}
```
...
public class PizzaController {
    ...
    public PizzaController(PizzaService pizzaService) {
        ...
        log.info("PizzaController initialized");
    }
    
    public void displayMenu() {
        log.debug("Displaying pizza menu");
        ...
        try {
            ...
            log.trace("Menu displayed successfully with {} items", pizzas.size());
        } catch (Exception e) {
            log.error("Error displaying menu", e);
            System.out.println("Sorry, could not load menu");
        }
    }
}
```
